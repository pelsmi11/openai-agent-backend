...
**Agent system prompts in `agent-graph.ts` are written in English, deliberately** — even though the DB `content`/`category` data is Spanish and most real users are Spanish-speaking. Any Spanish text in the system prompt (including the model's own persona description) measurably pulled final answers toward Spanish regardless of what language the user wrote in, because the model tends to paraphrase/mirror the language of whatever dominates its context. Actual reply language is governed entirely by `LANGUAGE_RULE`, which instructs the model to always match the user's message language (verified against English/Spanish/German/French). For the same reason, `searchPersonalInfoTool`'s result in `agent-tools.ts` (the retrieved DB content, which *is* in Spanish) is prefixed with an explicit "translate before replying" instruction — without it, that retrieved Spanish content alone was enough to override `LANGUAGE_RULE` for the rest of the answer. Keep new system-prompt text and tool descriptions/error strings in English for this reason; the confirmation email HTML in `agent-tools.ts` is a deliberate exception (it's user-facing content sent to the meeting attendee, not part of the chat reply pipeline) and stays in Spanish.
